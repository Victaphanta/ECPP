...
def parse_args():
    ...
    parser.add_argument("-aligner", dest = "aln_exec", default = "linsi --quiet", type = str, help="Any mafft algorithm (mafft, linsi, ginsi, einsi...). Default is linsi.")
    parser.add_argument("-options", dest="aln_opt", default = " --quiet --thread -1 --maxiterate 1000 --genafpair ", type = str, help = "Optional mafft options.")
...
def translate_records(nuc_dict):
    ...
    for entry in nuc_dict:
        nuc_dict[entry] = nuc_dict[entry].replace('-','N') # Remove any possible gaps in nucleotide sequence
        ...
        for i in range(0,len(nucseq),3):
            codon = nucseq[i:i+3]
        #    try:
        #   amino = str( Seq.Seq(codon, generic_dna).translate(table=1) )
            amino = str( Seq.Seq(codon, generic_dna).translate(table=1,stop_symbol= ' S ' ) )
        #    except:

...
#                                                                                                 #
# EMAP 1.0.0-beta                                                                                 #
# Exon capture pipline, from raw reads to final alignments                                        #
# Developed by Adnan Moussalli (Museum Victoria, Australia)                                       #
#                                                                                                 #
###################################################################################################
shopt -s expand_aliases
source /etc/profile

TEMP=`getopt -o m:s:i:e:r:p:x:t: --long argm:,args:,argi:,arge:,argr:,argp:,argx:,argt: -n 'emap.sh' -- "$@"`
eval set -- "$TEMP"
...
for i in "${S[@]}"
do
	echo "DEDUPLICATE paired reads $i"
	printf '%s\t' "$(date)" > $PWD/1_reads/$i.dupsum.PE.log && 
	printf '%s\t' $i >> $PWD/1_reads/$i.dupsum.PE.log && 
	printf '%s\t' "$(zgrep '\s1:' $ARG_R/$i\_R1.fastq.gz | wc -l)" >> $PWD/1_reads/$i.dupsum.PE.log &&

	dedupe.sh in1=$ARG_R/$i\_R1.fastq.gz in2=$ARG_R/$i\_R2.fastq.gz out=$PWD/1_reads/DR/$i\.deduped.fastq.gz pigz=t unpigz=t overwrite=true
	reformat.sh in=$PWD/1_reads/DR/$i\.deduped.fastq.gz out1=$PWD/1_reads/DR/$i\_R1.DR.fastq out2=$PWD/1_reads/DR/$i\_R2.DR.fastq pigz=t unpigz=t overwrite=true
	grep '\s1:' $PWD/1_reads/DR/$i\_R1.DR.fastq | wc -l >> $PWD/1_reads/$i.dupsum.PE.log &&
	rm  $PWD/1_reads/DR/$i\.deduped.fastq.gz


	# subset reads to only those having a hit to reference
	echo "LASTAL subset paired reads $i"
	...
	seqtk subseq $PWD/1_reads/DR/$i\_R1.DR.fastq $PWD/1_reads/DR/$i\.HITS.PE.txt > $PWD/1_reads/DRSUB/$i\_R1.DRSUB.fastq &&
	seqtk subseq $PWD/1_reads/DR/$i\_R2.DR.fastq $PWD/1_reads/DR/$i\.HITS.PE.txt > $PWD/1_reads/DRSUB/$i\_R2.DRSUB.fastq &&
	...
	paste $PWD/1_reads/$i.dupsum.PE.log $PWD/1_reads/$i.subtrimsumPE.log > $PWD/1_reads/$i.dupsubtrimsumPE.log
...
do
	...
	dedupe.sh in=$ARG_R/$i\_RS.fastq.gz out=$PWD/1_reads/DR/$i\_RS.DR.fastq pigz=t unpigz=t overwrite=true
...
cat $PWD/1_reads/*.dupsubtrimsumPE.log > $PWD/7_sum/dupsubtrimsumPE.log
cat $PWD/1_reads/*.dupsubtrimsumSE.log > $PWD/7_sum/dupsubtrimsumSE.log
fi	
...
do
	...
	# Concatenate paired and unpaired reads and assemble with TRINITY#
	if [ -f "$PWD/1_reads/DRTRIM/$i\_RS.DRTRIMSE.fastq.gz" ]; then
		...
		cat 	$PWD/1_reads/DRTRIM/$i\_R1.DRTRIMP.fastq.gz \
			$PWD/1_reads/DRTRIM/$i\_R1.DRTRIMU.fastq.gz \
			$PWD/1_reads/DRTRIM/$i\_R2.DRTRIMU.fastq.gz > $PWD/1_reads/DRTRIM/$i\_R1.DRTRIMPU.fastq.gz
	fi
	Trinity --seqType fq --max_memory 40G --min_contig_length 100 --no_normalize_reads \
		...
		--output $PWD/2_assemble/$i\.trinity --CPU $ARG_P --full_cleanup --min_kmer_cov 2 
	fi
...
do
	...
	if [ -f "$PWD/1_reads/DRTRIM/$i\_RS.DRTRIMSE.fastq.gz" ]; then
		...
		cat 	$PWD/1_reads/DRTRIM/$i\_R1.DRTRIMU.fastq.gz \
			$PWD/1_reads/DRTRIM/$i\_R2.DRTRIMU.fastq.gz > $PWD/1_reads/DRTRIM/$i\_R1.DRTRIMUP.fastq.gz
	fi
	
	bbwrap.sh threads=54 k=12 unpigz=t minratio=0.25 maxindel=100 local\
	ref=$PWD/4_map/$i.tblastn.top6.exons \
	in1=$PWD/1_reads/DRTRIM/$i\_R1.DRTRIMP.fastq.gz,$PWD/1_reads/DRTRIM/$i\_R1.DRTRIMUP.fastq.gz\
	in2=$PWD/1_reads/DRTRIM/$i\_R2.DRTRIMP.fastq.gz,null out=$PWD/4_map/$i.trinity_bbmap.sam  \
	append nodisk \
...
do
	...
	perl -p -e "s{^}{TTT} if $. %2 ==0" $PWD/6_align_subset/$DATE/${EN}_unaligned.fasta \
		> $PWD/6_align_subset/$DATE/${EN}_unalignedt.fasta && rm $PWD/6_align_subset/$DATE/${EN}_unaligned.fasta
...
# I REPEAT ESWEEP HERE SUCH THAT THE FINAL COMBINED.NEX PARTION INFORMATION IS UPDATED.
esweep_linux_v1 \
